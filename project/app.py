"""Streamlit UI for local-first feedback analytics."""
...
if uploaded_file:
    ...
    feedback_column = st.selectbox(
        "Select feedback column", options=df.columns, index=list(df.columns).index(detected_column) if detected_column else 0
    )
    ...
    aggregation_candidates = [
        ...
        if col.lower() in {"state", "district", "institute", "institution type", "institution_type"}
    ]
